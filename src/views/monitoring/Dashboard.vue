  ...
  <v-container class="pa-0">
    ...
    >
      <v-col cols="auto" style="background-color: #131419; height:100%; width:23%">
        ...
        <v-sheet dark color="transparent" height="20%">
        ...
        <v-sheet dark color="transparent" height="42%">
          <v-container fill-height class="text-body-2">
            ...
            <v-card class="rounded-card" color="#222631" height="95%" width="100%" >
              <v-container fluid>
                ...
                <template #item="{ data: item }">
                  <v-card
                    class="d-flex align-center rounded-lg my-1 px-3 "
                    height="calc(12vh - 66px)"
                    color="#2E3442"
                  >
                    <!-- <div>{{item.errorCd}}</div> -->
                    <!-- <v-icon color="#FF852F">{{'warning'}}</v-icon> -->
                    <v-img v-if="item.scNo == 'AUTO_LINE'"
                    ...
                    />
                    <v-img v-else
                      :src="require(`@/assets/images/kier/dashboard/stacker_crane.png`)"
                      contain
                      max-height="100%"
                      max-width="45"
                    />
                    <v-spacer/>
                    <div class="mx-2">{{item.errorDesc}}</div>
                    <v-spacer/>
        ...
        <v-sheet dark color="transparent" height="30%">
          <v-container fill-height class="text-body-2">
            <v-row>
                    ...
                    <v-card class="rounded-card" color="#2E3442" height="40%" width="100%">
                      <v-container fill-height class="pa-1 pa-lg-3">
                        ...
                        <v-badge
                          ...
                          dot
                          overlap
                          :value="purchasingOrder.hasWating"
                        >
                          <v-btn
                            ...
                            width="32px"
                            color="#3F475A"
                            @click="$router.push({ name:'PurchasingOrderManage',
                              params: {fromDashboard: true, hasWating: purchasingOrder.hasWating}})"
                          >
                      ...
                      <v-container fill-height class="pa-1 pa-lg-3">
                        ...
                        >
                          <v-btn
                            ...
                            width="32px"
                            color="#3F475A"
                            @click="$router.push({name: 'CustomerOrderManage',
                              params: {fromDashboard: true, hasWating: customerOrder.hasWating}})"
                          >
        ...
        <v-sheet dark color="transparent" height="calc(70% - 120px)">
          ...
          >
            ...
            <DxCustomShape
              ...
              :default-width="containerSize.width * 0.3"
              :default-height="containerSize.height * 0.4"
            />
            <DxCustomShape
              type="rack_1"
              :background-image-url="require(`@/assets/images/kier/dashboard/rack_1.png`)"
              :default-width="containerSize.width * 0.3"
              :default-height="containerSize.height * 0.4"
            />
            <DxCustomShape
              ...
              type="rack_2"
              :background-image-url="require(`@/assets/images/kier/dashboard/rack_2.png`)"
              :default-width="containerSize.width * 0.3"
              :default-height="containerSize.height * 0.4"
            />
            <!-- <DxCustomShape
            ...
            <DxCustomShape
              type="legend"
              :default-width="containerSize.width * 0.2"
        ...
        <v-sheet dark color="transparent" height="30%" width="100%">
          <v-container fill-height>
            ...
            <v-sheet class="pt-2" color="transparent" height="85%" width="100%">
              <v-card class="rounded-card d-flex justify-center align-center" color="#181C25" height="100%" width="100%"
      ...
      <v-col cols="auto" style="background-color: #0D0E13; height:100%; width:20%;">
        <v-sheet dark color="transparent" height="100%">
          <v-container fill-height>
                ...
                >
                  ...
                  >
                    <DxLabel
                      ...
                      :customize-text="(pointInfo) =>
                        (pointInfo.value ? `${pointInfo.valueText}\n(${pointInfo.percentText})` : '')"
            ...
            <v-card class="rounded-card pl-lg-3" height="15%" width="100%" style="background: linear-gradient(245deg, #47B9F8, rgba(71,185,248,0.2));">
              <v-container fill-height>
                <div class="d-flex flex-column justify-center" style="height: 100%; width: 40%;">
            ...
            <v-card class="rounded-card px-lg-3" height="14%" width="100%" color="#181C25" @click="$router.push({name:'MaterialInfo', params: {type: 'area'}})">
              <v-container fill-height>
            ...
            <v-card class="rounded-card px-lg-3" height="20%" width="100%" color="#181C25" @click="$router.push({name:'MaterialInfo', params: {type: 'auto'}})">
              <v-container fill-height>
            ...
            <v-card class="rounded-card px-lg-3" height="20%" width="100%" color="#181C25" @click="$router.push({name:'MaterialInfo', params: {type: 'manual'}})">
              <v-container fill-height>
                <v-row no-gutters align="center">
...
} from '@/api/kier/monitoring/dashBoard'
...
export default {
  ...
  computed: {
    rackRate() {
      let rate = (this.storageA.jjt + this.storageA.jjt) * 100 / ((this.storageA.jjt + this.storageA.jjt + this.storageA.gbt + this.storageA.gbt) || 130)
      return rate < 30 ? 0 : 
        rate < 50 ? 30 : 
        rate < 70 ? 50 : 
        rate < 100 ? 70 :
    ...
    rackRateManual() {
      return this.storageM.rate < 100 ? 0 : 1
    },
  },
  activated() {
    this.getDashboard()
    ...
    getDashboard() {
      getMonDashBoard({})
        .then( res => {
          ...
          this.cvRealStatus.sigStatus = map.cvRealStatus[0].sigStatus

          this.cvRealStatus.status = map.cvRealStatus[0].status
