              ...
              <v-col cols="12" md="12" class="row pa-0">
                <v-col cols="12" lg="12" class="pa-0">
                  ...
                  >
                    ...
                    <DxColumn data-field="useYn" data-type="boolean" caption="사용 여부" alignment="center" edit-cell-template="checkBoxEditor" />
  ...
  methods: {
    onEditorPreparing(e) {
      if (e.parentType == 'dataRow' && e.type != 'selection') {
        ...
        if (e.dataField == 'partnerId' || e.dataField == 'partnerNm') e.editorOptions.readOnlbtnSearchy = !e.row.data.isCreated
    ...
    doSearchMain(isProgress) {
      ...
      this.gridInit()
      return getPartner(params, isProgress).then((res) => {
    ...
    async btnSave() {
      ...
      for (var row of selectedMainRows) {
        ...
        if (!row.partnerType) {
          this.vToastify('필수항목 입력하세요.[파트너사 분류]', 'warn')
          return
        }
      }
