<template>
  <v-container fluid fill-height align-start ma-0 pa-2>
    ...
    <DxPopup :visible.sync="countVisible" :show-close-button="false" :show-title="false">
      <v-row dense align="center">
        <v-col cols="4">
          <v-autocomplete
            :menu-props="{ offsetY: true }"
        ...
        <v-spacer />
        <v-col cols="auto">
          <v-btn tile outlined color="btn_bg" height="30px" width="65px" @click="btnCount()">
      ...
      >
        ...
        <DxSummary>
          <DxGroupItem column="qty" summaryType="sum" valueFormat="#,##0" displayFormat="합 : {0}" :alignByColumn="true"> </DxGroupItem>
          <DxTotalItem column="qty" summary-type="sum" valueFormat="#,##0" display-format="총 수량 : {0}" />
        </DxSummary>
      </DxDataGrid>
    </DxPopup>
    ...
    <DxPopup
      ...
      @shown="
        e => {
          $refs[dxDataGridCellRef].instance.repaint()
        }
      "
    >
      <v-row dense align="center">
      ...
      >
        ...
        <DxColumn data-field="matDiv" caption="RACK 구분" data-type="string" width="90px" alignment="center">
          <DxLookup :data-source="matDivList" value-expr="code" display-expr="desc" />
        </DxColumn>
...
export default {
  ...
  beforeMount() {
    Promise.all([getComboStdMatManage(), getComboMultipalletIdWmsMaterialInfo(), getCommonCode('S0016'), getCommonCode('S0008')])
      .then(res => {
  ...
  methods: {
    ...
    doSearchMain() {
      ...
      return Promise.all([getWmsMaterialInfo(params, false), getWmsMaterialInfoArea()]).then(res => {
    ...
    repaint() {
      if (this.$refs[this.dxDataGridFirstRef]) this.gridFirstInstance.repaint()
      if (this.$refs[this.dxDataGridSecondRef]) this.gridSecondInstance.repaint()
    },

    beginUpdateAll() {
      if (this.$refs[this.dxDataGridFirstRef]) this.gridFirstInstance.beginUpdate()
      if (this.$refs[this.dxDataGridSecondRef]) this.gridSecondInstance.beginUpdate()
    },
    endUpdateAll() {
      if (this.$refs[this.dxDataGridFirstRef]) this.gridFirstInstance.endUpdate()
      if (this.$refs[this.dxDataGridSecondRef]) this.gridSecondInstance.endUpdate()
    }
  }
}
</script>
