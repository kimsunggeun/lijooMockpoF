              ...
              <v-col cols="12" md="12" class="pa-0">
                <v-col cols="12" lg="12" class="pa-0">
                  ...
                  >
                    ...
                    <DxColumn data-field="inQty" caption="실입고 수량" width="80px" data-type="number" format="#,##0" alignment="right" :allow-editing="false"/>

                    <DxColumn data-field="stockQty" caption="물류창고 입고 수량" width="120px" data-type="number" format="#,##0" alignment="right" :allow-editing="false"/>
                    ...
                    <DxColumn data-field="barcodeNo" caption="바코드 번호" data-type="string" width="180px" alignment="center" :allow-editing="false"/>
...
export default {
  ...
  methods: {
    ...
    doSearchMain(isProgress) {
      ...
      this.gridInit()
      return getWmsPalletInManageMain(params, isProgress)
        .then(res => {
      ...
      if(this.lastMonth[0] != null){
       if(this.focusata != null && (Number(this.focusata.substring(0,7).replace(/-/g,'')) <= Number(this.lastMonth[0].monthDt))){
        ...
        this.vToastify(this.$t(`저장할 수 없습니다.<BR>
          도착 실제일이 마지막 마감월인 ${this.lastMonth[0].monthDt.replace(/^(\d{0,4})(\d{0,4})(\d{0,4})$/g, "$1-$2-$3").replace(/-{1,2}$/g, "")} 월 이거나 전월입니다.
          `), 'warn')
        return
      }
      }
    ...
    async btnDelete() {
      ...
      if (!selectedDetailRows.length) {
        ...
        this.vToastify(this.$t(`삭제할 수 없습니다.<BR>
          도착 실제일이 마지막 마감월인 ${this.lastMonth[0].monthDt.replace(/^(\d{0,4})(\d{0,4})(\d{0,4})$/g, "$1-$2-$3").replace(/-{1,2}$/g, "")} 월 이거나 전월입니다.
         `), 'warn')
        return
      }
      ...
      this.vToastifyPrompt(
        ...
        current => {
          this.openLoading()
          this.gridDetailInstance.beginUpdate()
          deleteWmsPalletInManageDetail(selectedDetailRows.filter(row => !row.isCreated), true)
            .then(res => {
              this.doSearchDetail(this.focusedRowData.invoiceId)
            })
            .finally(() => {
    ...
    doSearchDetail(invoiceId) {
      ...
      return getWmsPalletInManageDetail(params, false)
        .then(res => {
          this.gridDetailInit()
