<template>
  <v-container fluid fill-height align-start>
    <v-row>
                ...
                <v-col cols="12" lg="12" class="pa-0">
                  ...
                  >
                    ...
                    <DxColumn data-field="hrId" caption="입하 담당자" data-type="string" alignment="center" edit-cell-template="hrId">
                      <DxLookup :data-source="hrManageList" value-expr="code" display-expr="desc" />
                    </DxColumn>
      ...
      <v-col cols="12" sm="12" lg="12" class="pa-2 pt-5 pt-lg-0">
        <i-card-vertical headerTitle="거래명세서 품목 리스트" :useBtnList="['btnSave']" @btnSave="btnSaveDetail()">
          <template v-slot:switch>
            <v-switch class="ma-1" v-model="delYnDetail" true-value="Y" false-value="N" :label="` 삭제 데이터 조회`" @change="onDelYnDetailChange" />
          </template>
          <template v-slot:body>
            <v-layout column overflow-auto>
              <v-col cols="12" md="12" class="pa-0">
...
export default {
  ...
  methods: {
    ...
    doSearchMain(isProgress) {
      ...
      this.gridInit()
      return getCargoArrival(params, isProgress).then((res) => {
      ...
      for (var row of selectedMainRows) {
        ...
        if (this.lastMonth[0] != null && row.orgAta != null) {
          if (Number(row.orgAta.substring(0, 7).replace(/-/g, '')) <= Number(this.lastMonth[0].monthDt)) {
            ...
            도착 실제일이 마감월인 ${this.lastMonth[0].monthDt
    ...
    doSearchDetail(invoiceId) {
      ...
      return getCargoArrivalDetail(params, false).then((res) => {
        this.gridDetailInit()
