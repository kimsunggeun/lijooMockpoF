          ...
          <template v-slot:body>
            <v-row no-gutters class="center pa-2 pb-0">
              ...
              <span style="width: 164px" class="pa-2 py-md-0">
                <v-switch v-model="toggleDelYn" true-value="Y" false-value="N" :label="`삭제 데이터 조회`" @change="btnSearch()" />
              </span>
            </v-row>
          </template>
        </i-card-top>
              ...
              <v-col cols="12" md="12" class="row pa-0">
                <v-col cols="12" lg="12" class="pa-0">
                  ...
                  >
                    ...
                    <DxColumn data-field="cqty" caption="공급 수량" data-type="number" alignment="right" :allow-editing="false" format="#,##0" />
...
import { getScmCustomerOrderManageMain, getScmCustomerOrderInfoDetail, getComboPartnerId } from '@/api/kier/scm/customerOrderInfo'
...
export default {
  ...
  beforeMount() {
    Promise.all([getComboPartnerId(), getStdPartnerEmployee({}, false), getCommonCode('S0010'), getCommonCode('S0011'), getComboStdMatManage()])
      .then((res) => {
  ...
  methods: {
    ...
    doSearchDetail(orderId) {
      ...
      return getScmCustomerOrderInfoDetail(params, false).then((res) => {
        this.gridDetailInit()
