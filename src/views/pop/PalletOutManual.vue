          ...
          <template v-slot:body>
            <v-layout column class="grid pa-0 pt-0">
              ...
              >
                ...
                <DxColumn data-field="jobNo" caption="작업 지시서 번호" width="20%" data-type="string" alignment="center" css-class="devest-grid-header-require"/>
...
<script>
  ...
  import {
    getPalletOutManualMain,
    getPalletOutManualPopup,
    savePalletOutManualMain
  } from '@/api/kier/pop/palletOutManual'
  ...
  beforeMount() {
    ...
    methods: {
      ...
      onCellClick(cellInfo) {
        ...
        if(this.matInfoState[cellInfo.value].matExist)
          this.doSearchPopup()
        else
          this.vToastify(this.$t('재고가 없는 RACK 입니다.'), 'warn')
      },
      async doSearchMainList(isProgress) {
        ...
        return await getPalletOutManualMain(params, isProgress)
          .then(res => {
            let cnt = 1
      ...
      beginUpdateAll() {
        if(this.$refs[this.dxDataGridPopRef])
          this.gridFirstInstance.beginUpdate()
      },
      endUpdateAll() {
        if(this.$refs[this.dxDataGridPopRef])
