                    ...
                    <div class="listStyle">
                      ...
                      >
                        <template #item="{ data: item }">
                          <div>
                            <span style="font-size: 18pt; float: left; font-weight: bold; letter-spacing: 4px;" class="pl-4">
                              {{ item.PalletID }} ( Pallet 명 : {{ item.palletNm }} , RFID Tag : {{ item.rfidTag }} )</span
                            >
                          </div>
                        </template>
                      </DxList>
...
import { getPalletInManualMain, getPalletInManualPopup, savePalletInManualMain, deletePalletInManualMain } from '@/api/kier/pop/palletInManual'
...
export default {
  ...
  beforeMount() {
    Promise.all([getComboMultipalletIdWmsMaterialInfo()])
      .then(res => {
  ...
  methods: {
    dragContents(e) {
      ...
      if (e.cellElement.innerText !== '') {
        ...
        this.locationId = [{ row: this.locationIdPopup.slice(0, 2), col: this.locationIdPopup.slice(2, 4), lev: this.locationIdPopup.slice(4, 6) }]
        let params = {
        ...
        this.vToastifyPrompt(
          ...
          current => {
            ...
            deletePalletInManualMain(
              this.gridCell.filter(row => !row.isCreated),
              true
            ).then(res => {
              if (res.status === -500) {
    ...
    onCellClick(cellInfo) {
      ...
      if (this.matInfoState[cellInfo.value].matExist == false) {
        if (this.palletIdClick === '' || this.palletIdClick === undefined) {
          ...
          this.cellPopUpTitle =
            'RACK 정보(행/열/단) : ' +
            this.locationIdPopup.slice(0, 2) +
            ' / ' +
            this.locationIdPopup.slice(2, 4) +
            ' / ' +
            this.locationIdPopup.slice(4, 6)
          let params = {
            palletId: this.palletIdClick,
            rfidTag: this.rfidTagClick
          }
    ...
    doSearchMain(isProgress) {
      ...
      return getPalletInManualMain(params, isProgress).then(res => {
        for (let i = 0; i < res.listResponse.map.list.length; i++) {
          this.PalletInManualList.push({
            PalletID: res.listResponse.map.list[i].palletId,
            rfidTag: res.listResponse.map.list[i].rfidTag,
            palletNm: ''
          })
